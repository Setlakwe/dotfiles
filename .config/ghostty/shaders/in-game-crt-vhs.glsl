...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // --- Added VHS Wobbling Effect ---
    // Apply a subtle time-dependent offset to simulate VHS-style wobble.
    vec2 wobble = vec2(sin(iTime * 0.1 + uv.y * 5.0), cos(iTime * 0.1 + uv.x * 7.0)) * 0.002;
    uv += wobble;
    // ---------------------------------
    ...
    // --- Added Extra Light Random Noise ---
    // This adds a very subtle static overlay to further emulate the analog feel.
    float extraNoise = (fract(sin(dot(fragCoord.xy, vec2(12.9898,78.233)))*43758.5453) - 0.5) * 0.02;
    fragColor.rgb += extraNoise;
    // -----------------------------------------
